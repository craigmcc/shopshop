...
export const ListSidebar = async ({ list, profile }: ListSidebarProps) => {
  const role = list.members.find((member) => member.profileId === profile.id)
    ?.role;

  return (
    <aside className="flex h-full w-full flex-col items-center space-y-4 bg-[#F2F3F5] py-3 text-primary dark:bg-[#2B2D31]">
      <ListSidebarHeader list={list} profile={profile} role={role} />
      {!!list.members?.length && (
        <>
